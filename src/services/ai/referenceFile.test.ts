...
function createMockFile(name: string, content: string, type = "text/plain"): ReferenceFileLike {
  return {
    name,
    type,
    size: content.length
  } as ReferenceFileLike & { size: number };
}
...
describe("referenceFile helpers", () => {
  ...
  it("reads and merges reference files", async () => {
    ...
    try {
      const result = await addReferenceFiles([], [createMockFile("chapter.txt", "chapter content") as File]);
  ...
  it("replaces files with the same name", async () => {
    ...
    class MockFileReader {
      ...
      readAsText(file: ReferenceFileLike) {
    ...
    try {
      ...
      const result = await addReferenceFiles(existing, [createMockFile("notes.txt", "new") as File]);
  ...
  it("rejects when total chars exceed limit", async () => {
    ...
    try {
      const result = await addReferenceFiles([], [createMockFile("big.txt", "big") as File]);
  ...
  it("rejects when file count exceeds limit", async () => {
    ...
    const result = await addReferenceFiles(existing, [createMockFile("extra.txt", "a") as File]);
    expect(result.ok).toBe(false);
    if (!result.ok) {
