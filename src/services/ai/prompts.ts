...
export interface TutorOptionInput {
  key?: string;
  text?: string;
}
...
export interface TutorSerializedPayload {
  questionType?: string;
  stem?: string;
  texts?: string[];
...
export interface BuildQuestionTutorMessagesOptions {
  question: TutorQuestionInput | null | undefined;
  history?: ChatMessage[];
  userText?: string;
}
...
function buildExecutionInstructions(hasReferences: boolean): string {
  const lines = [
    "【执行说明】",
...
export function buildUserPrompt(prompt: unknown, references: ReferenceFile[] = []): string {
...
const ANSWER_REVIEW_RULES = `
复核规则：
- 独立判断每道题的答案是否正确；不要重写题目，不要生成新题。
- verdict 只能是 pass、fail、uncertain 三者之一：
  - pass：题干清楚，答案与题意一致，解析无明显错误。
  - fail：答案明显错误，或与题干/选项矛盾。
  - uncertain：信息不足、专业过偏、或你无法有把握判定。
- reason 用中文写 1 句，说明判定依据；不要输出标准答案改写版全文。
- 必须为输入中每一道题（按 index）给出一条评审；index 从 0 开始且与输入顺序一致。
- 只输出合法 JSON 对象，不要包裹 Markdown 代码块。
`.trim();

export function buildAnswerReviewSystemPrompt(): string {
  return `你是 _txt_ 题目答案复核助手。用户会提供已生成的填空题/单选题，请逐题核验答案与解析是否正确。

输出必须是合法 JSON，结构如下：
{
  "reviews": [
    { "index": 0, "verdict": "pass", "reason": "答案与题意一致" },
    { "index": 1, "verdict": "fail", "reason": "正确选项应为 B" },
    { "index": 2, "verdict": "uncertain", "reason": "题干信息不足以独立核验" }
  ]
}
...
export function buildAnswerReviewUserPrompt(questions: ReviewQuestionInput[] = []): string {
  ...
  return `【待复核题目】共 ${payload.length} 道。请按 index 逐题核验答案与解析，输出 reviews JSON。
...
export function buildQuestionTutorSystemPrompt(): string {
  return `你是 _txt_ 本题答疑助手。用户正在练习一道题，你需要基于下方题目数据协助理解与纠错。

${TUTOR_PRINCIPLES}`;
}

function hasQuestionImage(image: unknown): boolean {
  return typeof image === "string" && image.trim() !== "";
}

export function serializeQuestionForTutor(question: TutorQuestionInput | null | undefined): string {
  if (!question || typeof question !== "object") {
    return "{}";
  }

  const isMd5 = Boolean(question.MD5);
  const payload: TutorSerializedPayload = { MD5: isMd5 };
...
export function buildQuestionTutorMessages({
  question,
  history = [],
  userText
}: BuildQuestionTutorMessagesOptions): ChatMessage[] {
