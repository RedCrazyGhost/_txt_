...
declare global {
  ...
  var __openaiCreateMock: ReturnType<typeof vi.fn> | undefined;
}
...
describe("llmChatStream", () => {
  ...
  it("aggregates streamed reasoning and content", async () => {
    ...
    const result = await llmChatStream({
      ...
      messages: [{ role: "user", content: "hi" }],
      thinkingEnabled: true,
      onReasoningDelta: (full: string) => reasoningChunks.push(full),
      onContentDelta: (full: string) => contentChunks.push(full)
    });
