...
import {
  buildAnswerReviewSystemPrompt,
  buildAnswerReviewUserPrompt,
...
describe("buildAnswerReviewSystemPrompt", () => {
  it("describes review role, verdicts, and output shape", () => {
    const prompt = buildAnswerReviewSystemPrompt();

    expect(prompt).toContain("答案复核助手");
    expect(prompt).toContain("pass");
    expect(prompt).toContain("fail");
    expect(prompt).toContain("uncertain");
    expect(prompt).toContain('"reviews"');
    expect(prompt).toContain("不要重写题目");
    expect(prompt).toContain("填空题");
  });
});

describe("buildAnswerReviewUserPrompt", () => {
  it("embeds indexed questions for review", () => {
    const prompt = buildAnswerReviewUserPrompt([
      { txt: "1+1=_2_", answer: "2", explanation: "加法" },
      { txt: "2+2=_4_", answer: "4", explanation: "加法" }
    ]);

    expect(prompt).toContain("【待复核题目】");
    expect(prompt).toContain("共 2 道");
    expect(prompt).toContain('"index": 0');
    expect(prompt).toContain("1+1=_2_");
